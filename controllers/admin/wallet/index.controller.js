...
module.exports = {
    manageWallet: async function(req, res) {
        ...
        await config.helpers.permission(
            "manage_wallet",
            req,
            async(err, permissionData) => {
                await config.helpers.filter.walletFilter(
                    req,
                    async function(filterData) {
                        if (filterData != "" && req.method == "POST") {
                            return res.redirect("manage_wallet" + filterData);
                        }
                        res.render("admin/wallet/view", {
                    ...
                    async function(err, permissionData) {
                        for (i = 0; i < data.length; i++) {
                            ...
                            $but_view =
                                ...
                                "/view_wallet?id=" +
                                data[i].userId +
                                '" class="btn btn-flat btn-info btn-outline-primary" title="View"><i class="fas fa-eye"></i></a></span>';
    ...
    addWallet: async function(req, res) {
        if (req.method == "GET") {
            ...
            message = message.replace(
                "[DATETIME]",
                moment().format("D-M-YYYY hh-mm A")
            );
            message = message.replace(
                "[DATETIME]",
                moment().format("D-M-YYYY hh-mm A")
            ...
            if (wallet) {
                ...
                await Wallet.updateOne({ userId: mongoose.mongo.ObjectId(userId) },
                    walletData,
                    async function(err, data) {
                        if (err) {
                            console.log(err);
                        }
                        await config.helpers.sms.sendSMS(
                            userData,
                            slug,
                            message,
                            async function(smsData) {
                                req.flash("msg", {
                ...
                if (type == "Add") {
                    ...
                    wallet.save(async function(err, data) {
                        ...
                        await config.helpers.sms.sendSMS(
                            ...
                            async function(smsData) {
                                req.flash("msg", {
                                    msg: "Wallet has been Added Successfully",
                                    status: true,
                                });
                                res.redirect(config.constant.ADMINCALLURL + "/manage_wallet");
                                req.flash({});
                    ...
                    req.flash("msg", {
                        msg: "Add amount in wallet first",
                        status: false,
                    });
    ...
    viewWallet: async function(req, res) {
        if (req.method == "GET") {
            ...
            res.render("admin/wallet/walletdetail", {
                layout: "admin/layout/layout",
                pageTitle: pageTitle,
                moduleName: moduleName,
